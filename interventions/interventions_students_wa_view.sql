CREATE OR REPLACE VIEW illuminate.interventions_students_wa AS
WITH
...
WHERE
  ...
  AND (
    (
      SUBSTRING(courses.school_course_id, 1, 2) IN ('S3', 'S2')
      AND sites.site_name IN ('Summit Public School: Olympus')
    ) OR
    (
      (
        courses.long_name LIKE '%Solves (Int)%'
        OR courses.long_name LIKE '%Reads Level 3%'
      )
      AND sites.site_name IN ('Summit Public School: Sierra')
    )
  )
