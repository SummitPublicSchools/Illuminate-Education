/***********************************************************************************************************************
This query produces a roster of currently enrolled students in WA schools and indicates whether they are LAP
eligible. LAP eligibility is based on following criteria:
 - LAP Math: Scoring below 50th percentile on MAP Math AND earning a C+ or below in Math course from previous school year
 - LAP English: Scoring below 50th percentile on MAP Reading AND earning a C+ or below in English course from previous school year
If no MAP data is available, then only grades are used
If no grades are available, then only MAP score is used 
 
For MAP, the most recent score is used, if not available then the previous score is used.
...
SELECT
  ...
  , CASE
      WHEN ela_grade IS NULL THEN
        CASE WHEN map_reading.map_percentile < 50 THEN 1 ELSE 0 END
      WHEN map_reading.map_percentile IS NULL THEN
        CASE WHEN ela_grade IN ('C+', 'C', 'C-', 'D+', 'D', 'D-', 'F', 'INCOMPLETE') THEN 1 ELSE 0 END
      ELSE 
        CASE WHEN map_reading.map_percentile < 50
          AND ela_grade IN ('C+', 'C', 'C-', 'D+', 'D', 'D-', 'F', 'INCOMPLETE') THEN 1 ELSE 0 END
      END AS lap_reading_eligible
      
  , CASE
      WHEN math_grade IS NULL THEN
        CASE WHEN map_math.map_percentile < 50 THEN 1 ELSE 0 END
      WHEN map_math.map_percentile IS NULL THEN
        CASE WHEN math_grade IN ('C+', 'C', 'C-', 'D+', 'D', 'D-', 'F', 'INCOMPLETE') THEN 1 ELSE 0 END
      ELSE 
        CASE WHEN map_math.map_percentile < 50
          AND math_grade IN ('C+', 'C', 'C-', 'D+', 'D', 'D-', 'F', 'INCOMPLETE') THEN 1 ELSE 0 END
      END AS lap_map_eligible
FROM
  ...
  WHERE
    enrollments.session_id IN (148, 149, 150)
