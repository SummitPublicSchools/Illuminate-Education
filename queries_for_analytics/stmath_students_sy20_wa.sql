CREATE OR REPLACE VIEW illuminate.stmath_students_sy19_wa AS
WITH
...
WHERE
  ...
  AND (
    (
      ...
      AND courses.short_name LIKE '%Solves L%'
      AND (
        courses.short_name LIKE '%6th'
        OR courses.short_name LIKE '%7th'
      )
    )
    OR
    (
      sites.site_name IN ('Summit Public School: Atlas')
      AND timeblocks.timeblock_name LIKE '99.STMathGhostblock'
    )
    OR
    (
      sites.site_name IN ('Summit Public School: Olympus')
      AND courses.short_name LIKE '%Solves%'
    )
    OR
    (
      sites.site_name IN ('Summit Public School: Sierra')
      AND courses.long_name LIKE '%Solves (Int)%'
    )
  )