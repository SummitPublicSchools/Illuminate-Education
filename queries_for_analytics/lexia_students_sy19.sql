CREATE OR REPLACE VIEW illuminate.lexia_students_sy19_ca AS
WITH
...
WHERE
  ...
  AND (
    ...
    (site_name = 'Summit Public School: Rainier' AND courses.short_name LIKE '%Summit Reads%')
    OR (site_name = 'Summit Public School: Shasta' AND courses.short_name LIKE '%Reading Support%')
    OR (site_name = 'Summit Preparatory Charter High School' AND courses.short_name LIKE 'Summit Reads%')
    -- K2 Middle School
    OR (site_name = 'Summit Public School: K2' AND courses.short_name LIKE '%Reads%'
        AND timeblocks.timeblock_name LIKE '%Lexia Ghostblock%')
    -- K2 High School
    OR (site_name = 'Summit Public School: K2' AND courses.short_name LIKE 'Summit Reads%'
          AND timeblocks.timeblock_name LIKE '%(HS)%')
    OR (site_name = 'Summit Public School: Tahoma' AND courses.short_name LIKE '%Summit Reads%')
    OR (site_name = 'Summit Public School: Tahoma' AND courses.short_name LIKE '%LC-%')
    OR (site_name = 'Summit Public School: Tamalpais' AND courses.short_name LIKE 'Summit Reads%')
    OR (site_name = 'Everest Public High School' AND courses.short_name LIKE 'Summit Reads%')
    OR (site_name = 'Summit Public School: Denali' AND courses.short_name LIKE 'Summit Reads Level 3%')
    OR (site_name = 'Summit Public School: Denali' AND courses.short_name LIKE '%LC-Reading%')
  );
