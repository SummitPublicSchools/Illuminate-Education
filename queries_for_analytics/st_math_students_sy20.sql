CREATE OR REPLACE VIEW illuminate.stmath_students_sy19_ca AS
WITH
...
WHERE
  ...
  AND (
    -- select the specific sections for each school based off of Clever
    (site_name = 'Summit Public School: Rainier' AND courses.short_name LIKE '%Summit Solves%')
    OR (site_name = 'Summit Public School: Shasta' AND courses.short_name = 'Math I Support (Intervention)')
    OR (site_name = 'Summit Preparatory Charter High School' AND courses.short_name LIKE 'Summit Solves%'
        AND courses.short_name NOT LIKE '%12th%')
    -- K2 Middle School
    OR (site_name = 'Summit Public School: K2' AND courses.short_name LIKE 'Summit Solves%'
         AND timeblocks.timeblock_name LIKE '%ST Math Ghostblock%')
    -- K2 High School
    OR (site_name = 'Summit Public School: K2' AND courses.short_name LIKE 'Summit Solves%'
          AND timeblocks.timeblock_name LIKE '%(HS)%')
    OR (
      site_name = 'Summit Public School: Tahoma' AND courses.short_name LIKE '%Summit Solves%'
    )
    OR (
      site_name = 'Summit Public School: Tahoma' AND courses.short_name LIKE '%LC-%'
    )
    OR (site_name = 'Summit Public School: Tamalpais' AND courses.short_name LIKE 'Summit Solves%')
    OR (site_name = 'Everest Public High School' AND courses.short_name LIKE 'Summit Solves%')
    OR (site_name = 'Summit Public School: Denali' AND courses.short_name LIKE 'Summit Solves Level 3%'
         AND timeblocks.timeblock_name NOT LIKE '%(HS)%')
    OR (site_name = 'Summit Public School: Denali' AND courses.short_name LIKE '%LC-Math%')
  );
